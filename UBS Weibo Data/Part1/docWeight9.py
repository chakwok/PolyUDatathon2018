...
def getAllTestingDataset(): 
	with open('Status_Data9_with_city_code.json', 'r') as g: 
		data = json.loads(g.readline())
		for entry in data: 
			testingData.append(entry)
			appendedTestingData.append(entry['sid'])
...
def processForPrediction():
	...
	for entry in testingData:
		docDict[(entry['sid'])] = []
		for i in range(0, 13): 
			docDict[(entry['sid'])].append(computeScore(entry['s_text'], i))
	#print(len(docDict))
	#pp.pprint(json.dumps(docDict))
	print(len(docDict))
	with open('testingSet9.txt', 'w') as o:
		o.write(json.dumps(docDict))
